...
def main():
    ...
    direction = "right"
    time.sleep(DELAY)
    go_right(canvas, player_list, direction, goal, goal_position)
...
def go_right(canvas, player_list, direction, goal, goal_position):
    """
    ...
    while x < x_limit:
        ...
        if snake_length > 1:  # If there's more than one square
            for i in range(1, snake_length):  # move to previous i coordinates
                pre_x, pre_y = canvas.coords(player_list[i - 1])
                canvas.moveto(player_list[i], pre_x - SIZE, pre_y)
        # provando
        ...
        if direction == "right":
            ...
            break
    if direction == "left":
        go_left(canvas, player_list, direction, goal, goal_position)
    elif direction == "up":
        go_up(canvas, player_list, direction, goal, goal_position)
    elif direction == "down":
        go_down(canvas, player_list, direction, goal, goal_position)
    end_game(canvas, player_list)


def go_left(canvas, player_list, direction, goal, goal_position):
...
def go_down(canvas, player_list, direction, goal, goal_position):
    """move snake down"""
    snake_length = len(player_list)
    ...
    while y < y_limit:
        ...
        if snake_length > 1:  # If there's more than one square
            for i in range(1, snake_length):  # move to previous i coordinates
                pre_x, pre_y = canvas.coords(player_list[i - 1])
                canvas.moveto(player_list[i], pre_x, pre_y - SIZE)
        # provando
...
def go_up(canvas, player_list, direction, goal, goal_position):
    """move snake up"""
    snake_length = len(player_list)
